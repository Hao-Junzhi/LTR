...
from ptranking.metric.adhoc_metric import torch_nDCG_at_k, torch_nDCG_at_ks, torch_ap_at_k, torch_ap_at_ks
...
def map_at_k(ranker=None, test_data=None, k=10, gpu=False, device=None):
    '''
    There is no check based on the assumption (say light_filtering() is called) that each test instance Q includes at least k documents,
    and at least one relevant document. Or there will be errors.
    '''
    sum_map_at_k = torch.zeros(1)
    cnt = torch.zeros(1)
    already_sorted = True if test_data.presort else False
    for qid, batch_ranking, batch_labels in test_data: # _, [batch, ranking_size, num_features], [batch, ranking_size]
        if batch_labels.size(1) < k: continue # skip the query if the number of associated documents is smaller than k
        ...
        batch_map_at_k = torch_ap_at_k(batch_sys_sorted_labels=batch_sys_sorted_labels,
                                          batch_ideal_sorted_labels=batch_ideal_sorted_labels,
                                          k = k)

        sum_map_at_k += torch.squeeze(batch_map_at_k) # default batch_size=1 due to testing data
        cnt += 1

    avg_map_at_k = sum_map_at_k/cnt
    return avg_map_at_k

def map_at_ks(ranker=None, test_data=None, ks=[1, 5, 10], gpu=False, device=None):

    sum_map_at_ks = torch.zeros(len(ks))
    cnt = torch.zeros(1)
    already_sorted = True if test_data.presort else False
    for qid, batch_ranking, batch_labels in test_data: # _, [batch, ranking_size, num_features], [batch, ranking_size]
        if gpu: batch_ranking = batch_ranking.to(device)
        ...
        batch_map_at_ks = torch_ap_at_ks(batch_sys_sorted_labels=batch_sys_sorted_labels,
                                            batch_ideal_sorted_labels=batch_ideal_sorted_labels,
                                            ks=ks)

        # default batch_size=1 due to testing data
        sum_map_at_ks = torch.add(sum_map_at_ks, torch.squeeze(batch_map_at_ks, dim=0))
        cnt += 1

    avg_map_at_ks = sum_map_at_ks/cnt
    return avg_map_at_ks